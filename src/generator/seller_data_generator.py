...
class SellerDataGenerator(DataGenerator):
    ...
    def write(self, output_data: List[str]):
        ...
        try:
            ...
            self._log("INFO", f"Verkäuferdaten erfolgreich geschrieben: {full_path}")
        except IOError as e:
            self._log("ERROR", f"Fehler beim Schreiben der Verkäuferdaten nach {full_path}: {e}")
        except Exception as e:
            self._log("ERROR", f"Unerwarteter Fehler beim Schreiben der Verkäuferdaten: {e}")
    ...
    def generate(self, overall_tracker: Optional[ProgressTracker] = None):
        ...
        for index, main_number_data in enumerate(all_main_numbers_data):
            ...
            if not all([hasattr(main_number_data, 'is_valid'),
                        ...
                        hasattr(main_number_data, 'get_article_total')]): # Check name: get_article_total?
                 self._log("WARNING", f"Unerwartetes Datenobjekt bei Index {index}. Übersprungen.")
                 invalid_cnt += 1
                 continue
            ...
            try:
                ...
                self._log("ERROR", f"Kein Verkäufer für Index {index} (Hauptnummer {main_number_val}) gefunden.")
                invalid_cnt += 1
                continue # Skip if seller missing
            except AttributeError:
                 self._log("ERROR", "FleatMarket Objekt hat keine Methode 'get_seller_data'. Breche Schleife ab.")
                 invalid_cnt += total_items - processed_count + 1
                 if overall_tracker: overall_tracker.set_error(AttributeError("Missing get_seller_data"))
                 break # Abort loop
            except Exception as e:
                 self._log("ERROR", f"Unerwarteter Fehler beim Holen von Verkäuferdaten für Index {index}: {e}")
                 invalid_cnt += 1
                 continue
            ...
            if main_number_data.is_valid():
                try:
                    ...
                    valid_cnt += 1
                    self._log("DEBUG", f">> Verkäufer-Eintrag (OK): {first_name} {second_name}, MNr: {m_n}, Artikel: {a_q}, Wert: {a_t:.2f} EUR")
                except (ValueError, TypeError) as e:
                     self._log("ERROR", f"Datenkonvertierungsfehler für Hauptnummer {main_number_val}: {e}")
                     invalid_cnt += 1
                except Exception as e:
                     self._log("ERROR", f"Unerwarteter Fehler bei gültigem Eintrag {main_number_val}: {e}")
                     invalid_cnt += 1
            else:
                invalid_cnt += 1
                ...
                self._log("WARNING", f">> Verkäufer-Eintrag (UNGÜLTIG): {first_name} {second_name}, MNr: {m_n_str}, Artikel: {a_q_str}, Wert: {a_t_str}")
